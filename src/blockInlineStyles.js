...
const popEndingStyles = (styleStack, endingStyles) =>
  endingStyles.reduceRight((stack, style) => {
...
const characterStyles = (offset, ranges) =>
  ranges.filter((range) => offset >= range.offset && offset < range.offset + range.length)
...
const rangeIsSubset = (firstRange, secondRange) => {
  ...
  const secondEndWithinFirst =
    firstRange.offset + firstRange.length >= secondRange.offset + secondRange.length
...
const appendStartMarkup = (inlineHTML, string, styleRange) =>
  string + getElementHTML(inlineHTML(styleRange.style)).start

const prependEndMarkup = (inlineHTML, string, styleRange) =>
  getElementHTML(inlineHTML(styleRange.style)).end + string
...
export default (rawBlock, customInlineHTML = defaultCustomInlineHTML) => {
  ...
  result = styleStack.reduceRight(
    (res, openStyle) => res + getElementHTML(inlineHTML(openStyle.style)).end,
    result
  )
